...
from tabu_lab.experiments.query_row_finetune_lift import _new_row_model, _parameter_sha256
from tabu_lab.experiments.query_row_pretraining import load_query_row_pretrain_checkpoint
...
def _validation_baselines(task: Any, *, seed: int) -> dict[str, dict[str, float]]:
    ...
    def metrics(predicted_task_scale: np.ndarray) -> dict[str, float]:
        ...
        return {
            ...
            "mae": float(mean_absolute_error(truth_raw, predicted_raw)),
            "scaled_rmse": float(
                math.sqrt(mean_squared_error(task.response[task.validation_indices], predicted_task_scale))
            ),
            "r2": float(r2_score(truth_raw, predicted_raw)),
        }
...
def run_query_row_r3_diagnosis(
    *,
    ...
    for seed in seeds:
        for dataset_id in dataset_ids:
            ...
            for target_updates in update_checkpoints:
                ...
                pretrained_metrics = _validation_prediction(pretrained, task, device=resolved_device)
                records.append(
                    {
    ...
    result = {
        "schema_version": "tabu.query-row.r3-diagnosis.v1",
        ...
        "claim_boundary": "R3 corrected validation-only adaptation diagnosis; no accepted transfer claim",
