...
class BuildResult:
    ...
    def executable(self) -> bool:
        """Return whether this result contains a runnable model."""
...
class ModelVersionNotFoundError(RegistryError, KeyError):
    """An exact contract version is absent from the registry."""
...
@lru_cache(maxsize=1)
def _load_registry() -> tuple[ModelSpec, ...]:
    loaded: list[ModelSpec] = []
    for resource in sorted(_model_resource_dir().iterdir(), key=lambda item: item.name):
        if not resource.name.endswith((".yaml", ".yml")):
            continue
        try:
            raw = yaml.safe_load(resource.read_text(encoding="utf-8"))
            loaded.append(ModelSpec.model_validate(raw))
        except (OSError, yaml.YAMLError, ValidationError) as exc:
            message = f"invalid packaged manifest {resource.name}: {exc}"
            raise RegistryValidationError(message) from exc

    if not loaded:
        raise RegistryValidationError("no packaged model manifests found")

    ids = [spec.contract_id for spec in loaded]
    ...
    return tuple(sorted(loaded, key=lambda spec: spec.contract_id))
...
def list_model_versions(contract_id: str) -> tuple[ModelSpec, ...]:
    """Return the versions currently carried for one contract.

    PR1 intentionally carries one immutable version.  The explicit API avoids
    silently treating a future version as equivalent to ``0.2.0``.
    """
...
def get_model_spec(
    ...
    """Return the current contract or require an exact version match."""
    ...
    if contract_version is not None and spec.contract_version != contract_version:
...
def validate_registry_source_parity(
    *,
    ...
    """Fail unless public and packaged ModelSpecs have identical paths and bytes."""
