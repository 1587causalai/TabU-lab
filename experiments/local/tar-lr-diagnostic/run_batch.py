...
def main():
    ...
    spec_path = spec_dir / "lr1e-4.yaml"
    ...
    for dataset in ("iris", "diabetes"):
        for variant in ("lr1e-4", "lr3e-5"):
            spec_path = spec_dir / f"{variant}.yaml"
            spec = json.loads(spec_path.read_text())
            ...
            attempt = root / f"attempt-{dataset}-{variant}-{seed}"
            ...
            write("running")
            with (root / f"{dataset}-{variant}-{seed}.log").open("x") as log:
